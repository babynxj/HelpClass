<%@page import="classhelp.guofeng.common.FinishCheck"%>
...
<%
	...
	ArrayList fclist = tUtil.getFinishCheck(checkid);
...
<body>
			...
			<div class="wraper">
				<div class="nav">
					<ul>
						...
						<li class="nav-item cur"><a href="select_check.jsp?id=<%=id%>">发布考勤</a></li>
	...
	<div class="neirong">
		...
		<div class="neirongtxt2">
			<%
				...
				//记算总页数
				intRowCount = fclist.size(); //获取记录总数
				...
				if (intPageCount > 0) {
					...
					for (; i < (j + intPageSize); i++) {
						fc = (FinishCheck) fclist.get(i);
						stu = sUtil.getStudentInfo(fc.getStudentId());
			%>
			<div class="item2">
				<div class="itemtxt2">
					学号：<%=fc.getStudentId()%>
			...
			<div class="mypage" align="center">
				<a
					href="select_onecheck.jsp?checkid=<%=checkid%>&id=<%=id%>&page=<%=1%>"><input
					class="pagebutton" type="button" value="首页"></a> <a
					href="select_onecheck.jsp?checkid=<%=checkid%>&id=<%=id%>&page=<%=intPageCount%>"><input
 ...
 %>
				<a
					href="select_onecheck.jsp?checkid=<%=checkid%>&id=<%=id%>&page=<%=intPage + 1%>"><input
